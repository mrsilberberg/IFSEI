...
echo "=============================="
echo "  IFSEI Add-on - Feedback via MQTT (lógica corrigida) "
echo "=============================="
echo "IP: $IP"
echo "Porta: $PORT"
...
if ! mosquitto_pub -h "$MQTT_HOST" -p "$MQTT_PORT" -u "$MQTT_USER" -P "$MQTT_PASS"   -t "$TOPIC_PREFIX/test" -m "IFSEI MQTT connected at $(date)"; then
